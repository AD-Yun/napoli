...
public class DaumPoll extends KnowFilter {
	...
	static final Logger LOGGER = LoggerFactory.getLogger(DaumPoll.class);
	public DaumPoll() {
		...
		*/
		//HtmlParser.setExceptHost(new HashSet<String>(Arrays.asList("daumcdn.net/imgsrc.search/knowledge4/2009/poll/ico")));
		
		/*select target */
		fe.setSectionTarget(1);
		
		/* 답변 */
		//fe.setARegexp("<dd class=\"qContent\">(.*)<ul class=\"pollList clearfix\" id=\"pollList\">");
		fe.setARegexp("<dd class=\"qContent\">(.*)");
	...
	public Map<String, String> regexpPostprocessing(Map<String, String> result)  {
		...
		*//**
		...
		//본문에서 출처 삭제
		//String bodyhtml = result.get("a_bodyhtml");
		//bodyhtml = Pattern.compile("x-SCRIPT|scr-ipt|XSCRIPT", Pattern.CASE_INSENSITIVE).matcher(bodyhtml).replaceAll("script");
		
		//result.put("q_date_time","");
		
		String[] qInfos = result.get("id_readcount_datetime").split("<span class=\"bar\">\\|</span>");
		
		if(qInfos.length > 1) {
			/* writer */
			String id = qInfos[0];
			id = id.replaceAll("작성자:", "");
			id = id.replaceAll("엑스퍼트</b> :", "");
			result.put("id", id);
		...
		/* readcount */
		matcher = Pattern.compile("<span id=\"viewCount\">([0-9]+)", Pattern.MULTILINE | Pattern.DOTALL).matcher(result.get("readcount"));
        if(! matcher.find()) { 
        	result.put("filterstate", "2003");
        	return result;
        } else { 
        	result.put("readcount", matcher.group(1)); 
        }
	...
	public FilteredData setDataObject(Map<String, String> regexpResult) throws VeniceException {
		...
		//List<String> datetime = translateDate(regexpResult.get("q_date_time"));
		...
		//filteredResult.put("quesdate", datetime.get(0));
